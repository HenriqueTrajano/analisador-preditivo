        ...
        try (BufferedReader br = new BufferedReader(new FileReader(filename))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                for (int i = 0; i < auxReaderProd.length; i++) {
                    auxSP = new SimboloProducao(auxReaderProd[i].trim(), auxSG);

                    //Colcoca o first dentro da producao, se for algo que nao precise derivar
                    if (auxReaderProd[i].trim().substring(0,1).matches("[0-9]|[a-z]|E")) {
                        auxSP.adicionarFirst(auxReaderProd[i].trim().charAt(0));
                    }

                    auxSG.adicinarProducoes(auxSP);
                }
